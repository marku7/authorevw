			...
			<form action="addpost.asp" method="post">
				...
				<div class="form-group" align="right">
					...
					if filterBy = "Action" then
						whereClause = "WHERE genre = 'Action' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					elseif filterBy = "Comedy" then
						whereClause = "WHERE genre = 'Comedy' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					elseif filterBy = "Drama" then
						whereClause = "WHERE genre = 'Drama' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					elseif filterBy = "Romance" then
						whereClause = "WHERE genre = 'Romance' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					elseif filterBy = "Horror" then
						whereClause = "WHERE genre = 'Horror' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					elseif filterBy = "Fantasy" then
						whereClause = "WHERE genre = 'Fantasy' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					elseif filterBy = "Thrill" then
						whereClause = "WHERE genre = 'Thrill' AND (category = 'Book' OR category = 'Movie') AND tag = 1"
					else
						whereClause = "WHERE category = 'Book' OR category = 'Movie' AND tag = 1"
					end if
					...
					pid = rs("postid")
							...
							if hasReported Then
							  ...
							  response.write "<div class='report-icon'>"
							  response.write "<a class='report-button' data-bs-toggle='modal' data-bs-target='#reportModal' data-postid='"& pid &"'><img src='./assets/images/report.png' alt='' alt='' title='Report Post'></a>"
							  response.write "</div>"
							end if
						  end if
